...
namespace Facebook.Apps
{
    public class FacebookSessionClient
    {
        ...
        public async Task<FacebookSession> LoginAsync(string permissions)
        {
            ...
            try
            {
                var session = await FacebookSessionCacheProvider.Current.GetSessionData();
                if (session == null)
                {
                    // Authenticate
                ...
                else
                {
                    ...
                    var authResult = await PromptOAuthDialog(permissions, WebAuthenticationOptions.SilentMode);
                    if (authResult != null)
                    {
                ...
                // Save session data
                await FacebookSessionCacheProvider.Current.SaveSessionData(session);
        ...
        public async void Logout()
        {
            try
            {
                await FacebookSessionCacheProvider.Current.DeleteSessionData();
            }
            finally
            {
